...
import { MedicalHistoryDialog } from './medical-dialog'
...
export const MedicalHistory = ({data, isShowProfile} : DataProps) => {
    ...
    const renderRow = (item: ExtendedMedicalHistory) => {
        return (
            <tr
                key={item.id}
                className='border-b border-black even:bg-slate-50 text-sm hover:bg-salte-50'
            >
    ...
    return (
        <>
            <div className='bg-white rounded-xl p-2 2xl:p-6'>
                ...
                <Table colums={columns} renderRow={renderRow} data={data} />
            </div>
        </>
    )
}
